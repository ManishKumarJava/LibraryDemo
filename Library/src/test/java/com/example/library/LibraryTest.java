...
public class LibraryTest {
    ...
    @Test
    @ConditionalOnProperty(value = "myProperty.test.test", havingValue = "x2100x")
    public void getLibraryBooks() throws Exception {
        String uri = "/library/libBooks"; // http://localhost:8088/library/libBooks
        MvcResult mvcResult = mvc.perform(MockMvcRequestBuilders.get(uri)
                .accept(MediaType.APPLICATION_JSON_VALUE)).andReturn();
        int status = mvcResult.getResponse().getStatus();
        assertEquals(200, status);
        String content = mvcResult.getResponse().getContentAsString();
        System.out.println(" getLibraryBooks done");
    }
}
